...
'''this environment differ from the gym env since the action here are directly the frequency and the phase angles, the phase angles group are 27'''
class mClariSimplifiedEnv(gym.Env):
    metadata = {'render_modes': ['human', 'rgb_array']}

    def __init__(self):
        super(mClariSimplifiedEnv, self).__init__()
        self.task_number = 1
        self.viewer = None
        ...
        #create the array of phase angles it will be self.phase_angles
    ...
    def _get_obs(self, final_pose, initial_pose = None):
        ...
        observations = np.array([[mean_power,mean_velocity],list(final_pose)])
        observations = observations.reshape(-1)
        self.powers = np.array([0.0 for i in range(8)])
